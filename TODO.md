...
- [x] **F3** `/parse-image/stream` now has `stream.onAbort` → onDone skips `markPendingReady` AND `removePending(pendingName)` cleans the entry (premortem Group C found F3-alone left it stuck "parsing" forever; fixed). Tests: `app.inbox-collision.test.ts`.
- [x] **F5** `addPending` no-ops for a filename whose entry is `importing` → a re-upload can't hijack an in-flight claimed entry's path. Tests: `watcher-reupload.test.ts`.
- [x] **F2** `createTransaction` sends a deterministic YNAB `import_id` → an ack-lost retry of the same receipt is deduped by YNAB. Tests: `budget-ynab-importid.test.ts`.
- [x] **F1b** Stale-claim reaper: `claimedAt` + `reapStaleClaims` on the pending poll releases an `importing` claim with no terminal after 120s (verified safely > the provable max import duration: 30s/call cap, 429 doesn't sleep). Tests: `watcher-stale-claim.test.ts`.
- [x] **F8b** Resolved-bounded by F7/F9: the within-`findMatchingTransaction` api-key race is now bounded by shutdown clearing the client cache (next access rebuilds). A full per-import credential snapshot is a provider-interface refactor disproportionate to an ultra-narrow race that requires a config write to land between two specific awaits of one call. Same proportionality call as F12.

### Premortem residuals (tracked, not release-blocking)

- [ ] **import_id collision** (premortem Group C Bug 2): two genuinely-distinct receipts with identical merchant+date+amount collide on `import_id` → YNAB drops the second silently. Intentional, bounded — same semantics as bank-import dedupe; `createTransaction` lacks line items for a finer key. Surface to user; fix only if it bites (thread a per-receipt fingerprint).
- [ ] **parse not cancelled on abort** (premortem Group C Bug 3): F3 stops the data resurrection but `parseImageReceiptStream` keeps running on an abandoned file, pinning the llama-server slot. Belongs to the broader cancellation pass (AbortSignal through the pipeline).
